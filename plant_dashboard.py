...
def main():
    ...
    # Filter: Only keep orders with unshipped quantity > 0 (Focus on what plant manager needs to track)
    active_orders = orders_df[orders_df['unshipped_qty_num'] > 0].copy()

    # Select and rename key columns
    summary_df = active_orders[['order_no', 'customer_id', 'product_id', 'order_qty_num', 'unshipped_qty_num', 'produced_qty_num', 'promised_date', 'status']].copy()
    ...
    print(f"\n--- 廠務未交訂單追蹤看板 (篩選未交量 > 0，共 {len(summary_df)} 筆) ---")
    ...
    print(f"\n已成功更新廠務未交彙整報表至：{output_report_path}")
